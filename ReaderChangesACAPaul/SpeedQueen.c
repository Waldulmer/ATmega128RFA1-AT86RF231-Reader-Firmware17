...
bool getSQMachineStatus(void)
{
	...
	if (MdcStatus.MachineType[1] == ACA_SERIES)
	{
		temp[0] = ACA_STATUS_REQUEST_SIZE;
		temp[1] = ACA_STATUS_REQUEST;
	}
...
bool sendSQVendPrice(void)
{
	...
	if( MdcStatus.MachineType[1] == ACA_SERIES)
	{
		...
		temp[6] = 0; //topoff price low byte, dryer only
		temp[7] = 0; //topoff price high byte, dryer only
		temp[8] = MachineSetupData.TopoffTime; //minutes, dryer only
		temp[9] = 0; //topoff time seconds, dryer only
		temp[10] = MachineSetupData.CycleChangeResponse;
	}
	else if( MdcStatus.MachineType[1] == QUANTUM_SERIES)
	{
...
bool IsACA(void)
{
	u8 temp[] = {0x09, ACA_INIT_REQUEST, 0x00, 0x00, 0x00, __APP_REVISION__, __APP_MINOR__, READER_CONTROL_VEND_PRICE, VEND_DISPLAY_ENABLE, ACA_BAUD_9600};
...
bool getACAInitData()
{
	...
	if ( getSQData(buf) && buf[0] == ACA_INIT_PACKET)
	{
		//do something with the returned init data
		MachineSetupData.ProductByte[0] = buf[1];
		MachineSetupData.ProductByte[1] = buf[2];
		MachineSetupData.ProductByte[2] = buf[3]; 
		//MachineSetupData.ProductByte[3] = buf[4]; //additional product byte
		//MachineSetupData.ProductByte[4] = buf[5]; //additional product byte
		MachineSetupData.ControlConfig  = buf[6]; //5 total configuration bytes
		return true;
	}
	return false;
}
...
///Gets message from machine then responds with ACK/NAK
///Puts results at *buf
...
void initMachine(void)
{
	...
	{
		//set q flag
		MdcStatus.MachineType[1] = ACA_SERIES;
	}
	else if( isQuantum() )
	{
		//set q flag
